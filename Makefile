...
deploy-base: infra/aws/k8s.yml infra/aws/values-external-dns.yml
	...
	kapp deploy -y -a $(PREFIX_APPS)-base -f _ytt.2.yml
	@-rm -f _ytt.*.yml
...
deploy-contour: deploy-base
	...
	$(shell ytt -f _ytt.0.yml -f modules/contour/fix-ns.yml -f modules/contour/fix-ttlSecondsAfterFinished.yml  -f config -f config-custom >_ytt.1.yml)
...
deploy-harbor: deploy-base
	...
	kapp deploy -y -a $(PREFIX_APPS)-harbor -f _ytt.2.yml
	@-rm -f _ytt.*.yml
...
deploy-concourse: deploy-base
	...
	kapp deploy -y -a $(PREFIX_APPS)-concourse -f _ytt.2.yml
	@-rm -f _ytt.*.yml
...
deploy-jenkins: deploy-base
	...
	kapp deploy -y -a $(PREFIX_APPS)-jenkins -f _ytt.2.yml
	@-rm -f _ytt.*.yml
...
deploy-kubeapps: deploy-base
	...
	kapp deploy -y -a $(PREFIX_APPS)-kubeapps -f _ytt.2.yml
	@-rm -f _ytt.*.yml
...
deploy-artifactory: deploy-base
	...
	kapp deploy -y -a $(PREFIX_APPS)-artifactory -f _ytt.2.yml
	@-rm -f _ytt.*.yml
...
deploy-prometheus: deploy-base
	...
	kapp deploy -y -a $(PREFIX_APPS)-prometheus -f _ytt.2.yml
	@-rm -f _ytt.*.yml

deploy-kpack: deploy-base
	kapp deploy -y -a $(PREFIX_APPS)-kpack -f modules/kpack/cluster-stack.yml -f modules/kpack/cluster-store.yml -f https://github.com/pivotal/kpack/releases/download/v$(KPACK_VERSION)/release-$(KPACK_VERSION).yaml
